...
from simcore.monitoring.geometry import actor_box
...
class PairClearanceFrameRecorder(FrameRecorder):
    ...
    def record(self, sample: MonitorSample) -> dict[str, Any]:
        ...
        actor_a = find_actor(objects, actor_id_a) if actor_id_a is not None else None
        actor_b = find_actor(objects, actor_id_b) if actor_id_b is not None else None
        ...
        box_a = actor_box(actor_a)
        box_b = actor_box(actor_b)
        if box_a is None or box_b is None:
            values = {
                "center_distance_m": hypot(bx - ax, by - ay),
                "clearance_m": None,
        ...
        dx = box_b.center_x - box_a.center_x
        dy = box_b.center_y - box_a.center_y
